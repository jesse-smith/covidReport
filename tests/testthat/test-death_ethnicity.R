test_that("`death_plot_ethnicity()` matches doppelganger", {
  ...
  mockery::stub(
    death_plot_ethnicity,
    "death_calc_ethnicity",
    tbl_mock
  )
  ...
  plt <- death_plot_ethnicity(tibble::tibble(), date = "2021-04-01")

  mockery::expect_called(m_dt, n = 1L)
  suppressWarnings(
    vdiffr::expect_doppelganger(
      title = "death case rate by ethnicity",
      fig = plt,
      path = "death-plot-ethnicity"
    )
  )
})

test_that("`void(death_table_ethnicity())` matches snapshot", {
  tbl_mock <- mockery::mock(tibble::tribble(
    ~ grp, ~ n, ~ percent, ~ rate,
    "Hispanic/Latino", 700,       0.7,    0.1,
    "Not Hispanic/Latino", 100,       0.1,    0.1,
  ...
  tbl_void <- flextable::void(
    death_table_ethnicity(data, date = "2021-04-01"),
    part = "all"
  )
...
test_that("`death_calc_ethnicity()` matches snapshot", {
  ...
  tbl_death <- death_calc_ethnicity(data, date = "2021-04-01")
